...
Keep typography tight and editorial. English body/UI typography uses Inter, Arabic keeps IBM Plex Sans Arabic, and the English "Create" word uses a lighter Times-style italic treatment with tight tracking to match the requested reference. On mobile only, its tracking relaxes to preserve the desktop word-shape without letters colliding at the smaller display size.
...
The three framework videos now reference the September 14 R2 uploads in `Editing System/`: `001-Hook Attention.mp4.mp4`, `002-Retention Structure.mp4.mp4`, and `003-Watch time.mp4.mp4`. The doubled extension is the actual object key, not a typo to normalize. These new URLs replace the deleted single-extension objects and naturally create fresh browser cache keys. Shared English/Arabic content mapping, viewport playback and layout remain unchanged. All three public objects returned HTTP 200; desktop rendering was visually verified. No deployment or Figma review performed for this change.

## Optional Intro Video (2026-09-09)

Implemented: a persistent corner preview appears after 1.5 seconds on English and Arabic homepages, 300px wide on desktop and a compact 260px horizontal launcher on mobile. The X minimizes it to a 60px red play button, which restores the preview with keyboard focus preserved. It cannot be permanently dismissed; the earlier sessionStorage dismissal has been removed. Entrance motion, a periodic shine and a play-button pulse attract attention without moving the page layout. Reduced motion disables these effects and leaves a static poster. The preview is a locally generated six-second, silent 69 KB excerpt; the 33 MB full R2 video is requested only after an explicit click. The preview unmounts when minimized; the shared viewport owner pauses playback while the dialog is open or the tab is hidden.
